...
% div:= divisor for inter-frequency spacing
% fsplay:= sampling for sound player routine
...
% Correct machine-E innacurracies (checked only in version 2007b with 32-bit machine)
...
% Expand cells for isentropic encoding - cut cbytes in half for 32-bit machines 
...
% 0: do nothing, 1: odd -> even, -1: even -> odd
...
for i=1:steps
    ...
    for j=1:64
        ...
        if rfilter,
            ...
            sound( [keys( k, : ), fliplr(keys( k+1, : ))], fsplay )
        end        
        k = k + 1;
    end
end
